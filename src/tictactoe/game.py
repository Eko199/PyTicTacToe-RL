...
from src.saving.save_manager import save_json
...
class Game:
    ...
    def take_turn(self, big_x, big_y, small_x, small_y):
        ...
        """
        if self.next not in { (-1, -1), (big_x, big_y) } \
            or not self.board.play_turn(self.current_player, big_x, big_y, small_x, small_y):
    ...
    async def play_turn(self) -> tuple[int, int, int, int]:
        ...
        turn: tuple[int, int, int, int] | None = \
            self.players[self.current_player].get_turn(self.next, self.board)
    ...
    async def save(self, file_name: str | None = None) -> None:
        ...
        If it's not an autosave, calls the function for a successful save.
    ...
    def render(self, last_turn: tuple[int, int, int, int] | None = None) -> None:
        """
        Renders the game board in the appropriate environment. 
        Meant to be implemented in subclasses.

        Args:
            last_turn (tuple[int, int, int, int] | None, optional): 
                The last played move. Defaults to None.
        """

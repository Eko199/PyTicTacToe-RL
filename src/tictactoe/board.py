...
class Board:
    ...
    Also provides the big 3x3 board which can also have -1 as a value, 
    representing a small draw game.
    """
    EMPTY: int = 0
    FULL: int = -1
    ...
    def play_turn(self, player: int, big_x: int, big_y: int, small_x: int, small_y: int) -> bool:
        ...
        if self.big_board[big_y, big_x] != self.EMPTY \
            or current_board[small_y, small_x] != self.EMPTY:
            return False
    ...
    def check_small_board_valid(self, big_x: int, big_y: int) -> bool:
        """
        Checks if the specified small board is valid.
        A small board is valid if it was not taken by a player and is not full.
    ...
    def is_full(self) -> bool:
        ...
        """
        return all(not self.check_small_board_valid(big_x, big_y)
                   for big_x in range(3) for big_y in range(3))

    def valid_moves(self, next_board: tuple[int, int]) -> \
        Generator[tuple[int, int, int, int], None, None] :
        """
        Generates valid moves for the specified small board.
        If next is (-1, -1) then for all small boards.
        ...
        return ((int(big_x), int(big_y), int(small_x), int(small_y))
                ...
                if next_board in { (-1, -1), (big_x, big_y) }
                    and self.big_board[big_y, big_x] == self.EMPTY)

    def to_string(self,
                  *,
                  last_turn: tuple[int, int, int, int] | None = None,
                  next_board: tuple[int, int] | None = None) -> str:
        """
        ...
        Args:
            ...
            next_board (tuple[int, int] | None, optional): 
                The next board to play on, used for coloring. Defaults to None.

        Returns:
            str: The string representation of the board.
        """
        last_big_x, last_big_y, last_small_x, last_small_y = last_turn \
        ...
        for big_y in range(3):
            for small_y in range(3):
                ...
                for big_x in range(3):
                    big_board_cell: int = self.big_board[big_y, big_x]

                    if self.player_symbols[big_board_cell] in { "X", "O" }:
                        mapped: str = big_str_map[self.player_symbols[big_board_cell], small_y]

                        result += Fore.GREEN + mapped + Style.RESET_ALL \
                    ...
                    board_str: list[str] = \
                        [self.player_symbols[cell]
                         if (cell := self.board[big_y, big_x, small_y, small_x]) != self.EMPTY
                            or next_board not in { (-1, -1), (big_x, big_y) }
                    ...
                    if (big_x, big_y, small_y) == (last_big_x, last_big_y, last_small_y):
                        board_str[last_small_x] = \
                            Fore.GREEN + board_str[last_small_x] + Style.RESET_ALL
